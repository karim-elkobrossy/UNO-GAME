...
class Card():
    ...
    def update_variables(self, img_file_name, hovering_height, hovering_dir, rotate, pos, dim, player_ID):
        ...
        self.image = pygame.image.load(f'{cards_path}/{img_file_name}').convert_alpha()
        ...
        if not self.is_played and not self.on_deck: self.x, self.y = pos
        ...
        if not compatible_effect:
            ...
            if self.player_ID == 0 and mouse_pos is not None:
                ...
                if self.hovering_dir == 'y':
                    self.image_rect = self.image.get_rect(topleft=(self.x, self.y+hovering_dist))
                else:
                    self.image_rect = self.image.get_rect(topleft=(self.x+hovering_dist, self.y))  
    ...
    def move_to_discard(self, all_cards, old_top_card, main_colour):
        ...
        self.image = pygame.image.load(f'{cards_path}/{self.img_file_name}').convert_alpha()
